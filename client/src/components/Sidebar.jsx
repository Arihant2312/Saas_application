...
import { House, Icon, LogOut } from 'lucide-react'
import { SquarePen, Image, Eraser, Hash, Scissors, FileText, Users ,Music,BookOpenText} from 'lucide-react'
    ...
    return (
        <div className={`w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center max:sm:absolute top-14 bottom-0 ${sidebar
            ...
            <div className="my-7 w-full ">
                <img src={user?.imageUrl} alt="useravatar" className='w-13 rounded-full mx-auto' />
                <h1 className='mt-1 text-center'>{user?.fullName}</h1>
                <div className='px-3 mt-5 font-medium text-gray-700'>{navItems.map(({to,label,Icon})=>(
                  <NavLink
  key={to}
  to={to}
  end={to === '/ai'}
  ...
  {label}
            ...
            <div className="w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between">
                <div className='flex gap-2 items-center cursor-pointer' onClick={openUserProfile}>
                    <img src={user.imageUrl} className='w-8 rounded-full' alt="userimage" />
                    <div className="">
